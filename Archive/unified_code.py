...
class DataConverter:
    ...
    def convert(self, test_mode: bool = False) -> None:
        ...
        В обоих случаях результат сохраняется в итоговом JSON-файле (если merge_output=True).
        """
        ...
        try:
            ...
            # Объединение всех обработанных чанков в итоговый JSON, если это предусмотрено режимом вывода
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-mode", choices=["merge", "chunks"], default="merge", help="Output mode: merge into one file or keep chunks")
    ...
    converter = DataConverter(
        ...
        merge_output=True,  # для теста объединяем чанки
        **reader_kwargs
    )
    ...
    print("- База данных чекпоинтов сохранена в WIP/checkpoint.db.")
    print("- Логи доступны в папке logs/.")

    # Оптимизация тестового JSON
    test_optimize_choice = input("Хотите оптимизировать тестовый JSON? (Y/N): ").strip().lower()
    if test_optimize_choice == "y":
        test_output_path = OUTPUT_DIR / "test_output.json"
        optimized_test_path = OUTPUT_DIR / "test_output_optimized.json"
        try:
            stats = optimize_json(test_output_path, optimized_test_path)
            print("\nСтатистика оптимизации тестового JSON:")
            print(f"- Исходный размер: {stats['original_size_mb']:.2f} MB")
            print(f"- Размер после оптимизации: {stats['optimized_size_mb']:.2f} MB")
            print(f"- Сэкономлено места: {stats['saved_space_mb']:.2f} MB")
            print(f"- Уменьшение размера: {stats['reduction_percent']:.1f}%")
        except Exception as e:
    ...
    if full_run == "y":
        ...
        # Определяем режим объединения чанков в зависимости от выбранного output_mode
        merge_output = True if args.output_mode == "merge" else False
        ...
        # Оптимизация в зависимости от режима вывода
        if args.output_mode == "merge":
            print("Обработка завершена. Результаты сохранены в Output/output.json.")
            optimize_choice = input("Хотите оптимизировать итоговый JSON? (Y/N): ").strip().lower()
            if optimize_choice == "y":
                input_path = OUTPUT_DIR / "output.json"
                output_path = OUTPUT_DIR / "output_optimized.json"
                try:
                    stats = optimize_json(input_path, output_path)
                    print("\nСтатистика оптимизации итогового JSON:")
                    print(f"- Исходный размер: {stats['original_size_mb']:.2f} MB")
                    print(f"- Размер после оптимизации: {stats['optimized_size_mb']:.2f} MB")
                    print(f"- Сэкономлено места: {stats['saved_space_mb']:.2f} MB")
                    print(f"- Уменьшение размера: {stats['reduction_percent']:.1f}%")
        ...
        elif args.output_mode == "chunks":
            print("Чанки сохранены в WIP/ для дальнейшей оптимизации.")
            optimize_chunks_choice = input("Хотите оптимизировать все чанки? (Y/N): ").strip().lower()
            if optimize_chunks_choice == "y":
                chunks_dir = OUTPUT_DIR / "chunks"
                chunks_dir.mkdir(exist_ok=True)
                logger = MultiProcessLogger(Path("logs")).get_logger("optimizer")
                for chunk_file in sorted(WIP_DIR.glob("chunk_*.json")):
                    chunk_index = int(chunk_file.stem.split("_")[1])
                    optimized_chunk_path = chunks_dir / f"chunk_{chunk_index}_optimized.json"
                    try:
                        stats = optimize_json(chunk_file, optimized_chunk_path)
                        logger.info(f"Чанк {chunk_index} оптимизирован:")
                        logger.info(f"Исходный размер: {stats['original_size_mb']:.2f} MB")
                        logger.info(f"Размер после оптимизации: {stats['optimized_size_mb']:.2f} MB")
                        logger.info(f"Сэкономлено места: {stats['saved_space_mb']:.2f} MB")
                        logger.info(f"Уменьшение размера: {stats['reduction_percent']:.1f}%")
                    except Exception as e:
                        logger.error(f"Ошибка при оптимизации чанка {chunk_index}: {e}")
                print("Оптимизированные чанки сохранены в Output/chunks/.")
    else:
